...
{
	/// <summary>
	/// Treida popisujuca informacie EURONA pouzivatela
	/// </summary>
	public class EuronaUserMarzeInfo
	{
		...
		public EuronaUserMarzeInfo(System.Web.UI.Page page, int tvdId)
		{
			this.tvdId = tvdId;
		...
		public bool GetMarzeInfoFromEurosap(int? cartId, out string errorMessage)
		{
			...
			if (recalculate || hasSessionValue == false)
			{
				...
				if ( /*bSuccess == false && */zprava != null && zprava.Value != null)
				{
					int instanceId = 0;
					...
					string msg = string.Format("{0}:EURONA User -> esp_www_marze(Id_odberatele:{1}) = {2}", instnaceType.ToString(), this.tvdId, zprava.Value.ToString());
		...
		/// <summary>
		/// Metoda upravi UserMargin Info podla aktualnych EURONA produktov v kosiku
		/// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		/// V Session musia ostat povodne hodnoty z EUROSAP !!!!!!!
		/// </summary>
		private void UpdateByEuronaCart(int? cartId)
		{
			...
			try
			{
				...
				{
					storage.ExecProc(connection, "pShpRecalculateCernyForLifeCart",
