...
{
    public static class Storage<T> where T : class, new()
    {
        internal static IStorage<T> Instance
        {
            get
            {
                ...
                if (instance == null)
                    throw new InvalidOperationException(string.Format("Eurona.Common.Storage.Instance Can not be null! Please add storage definition for {0} int WebStorage",
                            typeof(T).ToString()));
