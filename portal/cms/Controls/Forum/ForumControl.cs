...
{
		public class ForumControl: CmsControl
		{
				...
				public ForumThreadEntity ForumThread
				{
						get
						{
								if (forumThread != null || ForumThreadId == null) return forumThread;
								forumThread = Storage<ForumThreadEntity>.ReadFirst(new ForumThreadEntity.ReadById { ForumThreadId = this.Forum.ForumThreadId });
								return forumThread;
						}
				}
				*/
				...
				private Control CreateDetailControl()
				{
						...
						//Save Cancel Buttons
						row = new TableRow();
						row.CssClass = this.CssClass + "_table_buttons";
						cell = new TableCell();
						cell.ColumnSpan = 2;
				...
				void OnSave( object sender, EventArgs e )
				{
						...
						this.Forum.Description = Server.HtmlEncode( this.txtDescription.Text );
						this.Forum.Locked = false;
						this.Forum.Pinned = false;

						if ( !this.ForumId.HasValue )
						{
								this.Forum = Storage<ForumEntity>.Create( this.Forum );
						...
						ForumThreadEntity forumThread = Storage<ForumThreadEntity>.ReadFirst( new ForumThreadEntity.ReadById { ForumThreadId = this.Forum.ForumThreadId } );
						string alias = string.Format( "{0}/{1}-{2}", forumThread.Alias, Utilities.AliasUtilities.GetAliasString( this.Forum.Name ), this.ForumId );
