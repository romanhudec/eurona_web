...
{
		public class AdminNavigationMenuControl: CmsControl
		{
				...
				protected override void CreateChildControls()
				{
						...
						else
						{
								...
								menuEntity = Storage<MenuEntity>.ReadFirst( new MenuEntity.ReadById { NavigationMenuId = menuId } );
						}
				...
				{
						List<UrlAliasEntity> urlAliasList = Storage<UrlAliasEntity>.Read().OrderBy( p => p.Name ).ToList();
						TableCell cell = new TableCell();
						cell.CssClass = "form_control";
				...
				private void CreateSaveButton()
				{
						...
						btnSave.Click += ( s1, e1 ) =>
						{
								...
								if ( isNew )
								{
										menuEntity.MenuId = Convert.ToInt32( Request["MenuId"] );
