...
{
	public static class CernyForLifeTVDDAL
	{
		...
		public const int LivienneKategorieId = 42;
		...
		public static DataTable GetTVDProducts(CMS.Pump.MSSQLStorage mssqStorageSrc, int instanceId)
		{
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				...
				{
					string sql = "SELECT DISTINCT C_Zbo, [Eurona_Produkt], [CL_Produkt] FROM CL_Produkty WHERE Eurona_Produkt=1";
					DataTable dt = mssqStorageSrc.Query(connection, sql);
					return dt;
				}
				else if (instanceId == 2) //Intensa
				{
					string sql = "SELECT DISTINCT C_Zbo, [Eurona_Produkt], [CL_Produkt] FROM CL_Produkty WHERE CL_Produkt=1";
					DataTable dt = mssqStorageSrc.Query(connection, sql);
					return dt;
				}
				...
				{
					string sql = "SELECT DISTINCT C_Zbo, [Eurona_Produkt], [CL_Produkt] FROM CL_Produkty";
					DataTable dt = mssqStorageSrc.Query(connection, sql);
					return dt;
				}
			}
		...
		public static DataRow GetTVDProduct(CMS.Pump.MSSQLStorage mssqStorageSrc, int productId)
		{
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				string sql = @"SELECT C_Zbo,[Kod],[Universal_nazev],[Vat],[Stock_Unit],[Bod_hodnota],
								...
								[Novinka],[Doprodej],[Inovace],[Prodej_Ukoncen],[Vyprodano], [Top_Produkt], [Eurona_Produkt], [CL_Produkt], [Dispozice_HR], [Dispozice_HR1],
								...
								FROM CL_Produkty WHERE C_Zbo=@ProductId";
		...
		public static bool IsLivienneProduct(CMS.Pump.MSSQLStorage mssqStorageSrc, int productId)
		{
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				string sql = @"SELECT *
								FROM CL_Produkty_Kategorie pk
								WHERE pk.C_Zbo=@ProductId AND Kategorie_Id IN (SELECT Kategorie_Id FROM CL_Kategorie WHERE Kategorie_Id=@KategorieLivienne OR Kategorie_Parent=@KategorieLivienne)";
				DataTable dt = mssqStorageSrc.Query(connection, sql,
						new SqlParameter("@ProductId", productId),
						new SqlParameter("@KategorieLivienne", CernyForLifeTVDDAL.LivienneKategorieId));
		...
		public static bool IsLivienneCategory(CMS.Pump.MSSQLStorage mssqStorageSrc, int categoryId)
		{
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				string sql = @"SELECT Kategorie_Id FROM CL_Kategorie 
					WHERE Kategorie_Id = @CategoryId AND (Kategorie_Id=@KategorieLivienne OR Kategorie_Parent=@KategorieLivienne)";
				DataTable dt = mssqStorageSrc.Query(connection, sql,
		...
		public static DataTable GetTVDProductPiktogramy(CMS.Pump.MSSQLStorage mssqStorageSrc, int productId, int jazyk)
		{
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				string sql = @"SELECT ppg.C_Zbo, pg.Jazyk, Name=pg.Popis, ImageUrl=''
								FROM CL_Produkty_Piktogramy ppg
		...
		public static DataTable GetTVDProductUcinky(CMS.Pump.MSSQLStorage mssqStorageSrc, int productId, int jazyk)
		{
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				string sql = @"SELECT psu.C_Zbo, sup.Jazyk, Name=sup.Popis, ImageUrl=su.Obrazek, su.Spec_Ucinek_Kod
										FROM CL_Produkty_Specialni_Ucinky psu
										INNER JOIN CL_Specialni_Ucinky su ON su.Spec_Ucinek_Kod = psu.Spec_Ucinek_Kod
		...
		public static DataTable GetTVDProductVlastnosti(CMS.Pump.MSSQLStorage mssqStorageSrc, int productId, int jazyk)
		{
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				string sql = @"SELECT pv.C_Zbo, pv.Jazyk, Name='', ImageUrl=pv.Obrazek
										FROM CL_Produkty_Vlastnosti pv
		...
		public static DataTable GetTVDProductAlternativni(CMS.Pump.MSSQLStorage mssqStorageSrc, int productId)
		{
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				string sql = @"SELECT C_Zbo, Alternativni_C_Zbo FROM CL_Produkty_Alternativni WHERE C_Zbo=@ProductId";
				DataTable dt = mssqStorageSrc.Query(connection, sql, new SqlParameter("@ProductId", productId));
		...
		public static DataRow GetTVDProductCeny(CMS.Pump.MSSQLStorage mssqStorageSrc, int productId, int jazyk)
		{
			...
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				string sql = @"SELECT pc.C_Zbo, pc.Mena, Kod=@Kod, epc.Body, pc.Cena, pc.Bezna_cena, pc.Dynamicka_sleva , pc.Staticka_sleva, pc.Cena_BK
										FROM CL_Produkty_Ceny pc
										LEFT JOIN Produkty_Ceny epc ON epc.C_Zbo = pc.C_Zbo AND epc.Mena = pc.Mena
		...
		public static DataTable GetTVDClsSpecialniUcinky(CMS.Pump.MSSQLStorage mssqStorageSrc)
		{
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				string sql = @"SELECT Spec_Ucinek_Kod, Obrazek FROM CL_Specialni_Ucinky";
				DataTable dt = mssqStorageSrc.Query(connection, sql);
		...
		public static DataTable GetTVDClsParfemace(CMS.Pump.MSSQLStorage mssqStorageSrc)
		{
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				string sql = @"SELECT Parfemace_Id, Obrazek FROM CL_Parfemace";
				DataTable dt = mssqStorageSrc.Query(connection, sql);
		...
		public static DataTable GetTVDProductKategorie(CMS.Pump.MSSQLStorage mssqStorageSrc, int productId, int instanceId)
		{
			...
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				string sql = @"SELECT C_Zbo, Kategorie_Id, Shop FROM CL_Produkty_Kategorie WHERE C_Zbo=@ProductId AND Shop=@Shop";
				DataTable dt = mssqStorageSrc.Query(connection, sql,
						new SqlParameter("@ProductId", productId),
		...
		public static DataTable GetTVDCategories(CMS.Pump.MSSQLStorage mssqStorageSrc)
		{
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				string sql = @"SELECT k.Kategorie_Id, k.Kategorie_Parent, k.Shop from CL_Kategorie k ORDER BY k.Kategorie_Parent ASC";
				DataTable dt = mssqStorageSrc.Query(connection, sql);
				return dt;
			}
		}
		...
		public static DataRow GetTVDCategory(CMS.Pump.MSSQLStorage mssqStorageSrc, int instanceId, int categoryId, int jazyk)
		{
			...
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				string sql = @"SELECT k.Kategorie_Id, k.Kategorie_Parent, kn.Jazyk, kn.Nazev, NazevParent = knp.Nazev from CL_Kategorie k
								INNER JOIN CL_Kategorie_Nazvy kn ON kn.Kategorie_Id = k.Kategorie_Id
								LEFT JOIN CL_Kategorie_Nazvy knp ON knp.Kategorie_Id = k.Kategorie_Parent
		...
		public static DataTable GetTVDFinalOrders(CMS.Pump.MSSQLStorage mssqStorageSrc, int instanceId)
		{
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				string sql = @"SELECT orderId = fa.id_prepoctu, fa.cislo_objednavky_eurosap, fa.potvrzeno, obfa.StavK2, obfa.Kod_meny, fa.sdeleni_pro_poradce_html
								FROM cl_objednavkyfaktury obfa
								INNER JOIN cl_www_faktury fa ON fa.cislo_objednavky_eurosap=obfa.Id_objednavky
		...
		public static void UpdateTVDPoradciStav(CMS.Pump.MSSQLStorage mssqStorageSrc, int poradcaId, int stav)
		{
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				string sql = @"UPDATE cl_odberatele SET Stav=@Stav WHERE Id_odberatele = @OderatelId";
		...
		public static DataTable GetTVDPoradci(CMS.Pump.MSSQLStorage mssqStorageSrc)
		{
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				string sql = @"SELECT 
					...
					FROM cl_odberatele WHERE Login_www IS NOT NULL AND Login_www != '' AND Stav_odberatele!='Z' AND Stav=1";
				DataTable dt = mssqStorageSrc.Query(connection, sql);
		...
		public static DataRow GetTVDPoradca(CMS.Pump.MSSQLStorage mssqStorageSrc, int poradcaId)
		{
			using (SqlConnection connection = mssqStorageSrc.Connect())
			{
				string sql = @"SELECT 
					...
					Kod_oblasti ,Datum_narozeni ,Ar_na_dor_adr ,Leadersky_titul ,Telefon_prace ,Fax ,Icq,
					Skype ,Zakazat_www ,Ar_jen_1_strana ,Poznamka ,Stav, Top_manazer
					FROM cl_odberatele WHERE Id_odberatele = @OderatelId";
