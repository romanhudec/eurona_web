...
namespace Cron.Eurona.Import {
    public class EuronaProductSynchronize : Synchronize {
        ...
        public EuronaProductSynchronize(int instanceId, int productId, string srcTVDImagePath, string dstProductImagePath, string dstVlastnostiImagePath, string dstPiktogramyImagePath, string dstZadniEtiketyImagePath,
            ...
            using (SqlConnection connection = this.DestinationDataStorage.Connect()) {
                ...
                try {
                    ...
                    try {
                        ...
                        bool isProductValid = EuronaTVDDAL.IsTVDProductLocalized(this.SourceDataStorage, this.ProductId);
                        if (!isProductValid) {
                        ...
                        DataRow drProduct = EuronaTVDDAL.GetTVDProduct(this.SourceDataStorage, this.ProductId);
                        DataTable dtKategorie = EuronaTVDDAL.GetTVDProductKategorie(this.SourceDataStorage, this.ProductId, this.InstanceId);
                        ...
                        bool intensaProduct = Convert.ToBoolean(drProduct["Intensa_Produkt"]);
                        ...
                        foreach (int jazyk in EuronaTVDDAL.TVDJazyky) {
                            DataRow drLocalize = EuronaTVDDAL.GetTVDProductLocalize(this.SourceDataStorage, this.ProductId, jazyk);
                            ...
                            //    );
                            //}
                            if (drcCeny != null && drcCeny.Count != 0) {
                            ...
                            if (dtPiktogramy != null) {
                                ...
                                foreach (DataRow drPiktogramy in dtPiktogramy.Rows) {
                                    ...
                                    string imageName = GetString(drPiktogramy["Name"]);
                        ...
                        OnItemProccessed(rowIndex, rowsCount, string.Format("Proccessing product images count:'{0}' : ok", images.Count));
        ...
        /// </summary>
        private void ImportProductImages(int itemId, string productKod, List<string> imageNames) {
            ...
            int imageCode = 0;
            if (imageNames.Count == 0 && productKod != null)
                imageFiles.Add(Path.Combine(srcDirectory, productKod + ".jpg"));


            foreach (string srcFileName in imageNames) {
                string srcFilePath = srcFileName.Trim();
                if (string.IsNullOrEmpty(srcFilePath)) continue;
                string imgPath = Path.Combine(srcDirectory, srcFileName);
            ...
            foreach (char ch in chars) {
                ...
                if (File.Exists(imgPath) && !imageFiles.Contains(imgPath)) {
                    imageFiles.Add(imgPath);
                }
            }
