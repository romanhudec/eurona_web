...
namespace Cron.Eurona.Import {
    public class EuronaCategorySynchronize : Synchronize {
        private string dstCategoryImagePath = null;
        private string srcCategoryImagePath = null;
        private int instanceId = 0;
        private int categoryId = 0;
        private int? parentId = null;
        public EuronaCategorySynchronize(int instanceId, int categoryId, int? parentId, MSSQLStorage srcSqlStorage, MSSQLStorage dstSqlStorage)
                ...
                try {
                    ...
                    try {
                        ...
                        foreach (int jazyk in EuronaTVDDAL.TVDJazyky) {
                            DataRow drCategory = EuronaTVDDAL.GetTVDCategory(this.SourceDataStorage, this.InstanceId, this.CategoryId, jazyk);
                            if (drCategory == null) continue;
                            string locale = EuronaTVDDAL.GetLocale(jazyk);
                            EuronaDAL.Classifiers.SyncCategoryLocale(this.DestinationDataStorage, this.CategoryId, GetString(drCategory["Nazev"]), null, this.instanceId, locale);
                        }
