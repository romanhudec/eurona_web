...
namespace Cron.Eurona.Import
{
	...
	/// </summary>
	public class CernyForLifeAccoutsSynchronize : Synchronize
	{
		private int instanceId = 0;
		public CernyForLifeAccoutsSynchronize(int instanceId, MSSQLStorage srcSqlStorage, MSSQLStorage dstSqlStorage)
			...
			using (SqlConnection connection = this.DestinationDataStorage.Connect())
			{
				try
				{
					...
					try
					{
						...
						//TVD->CL
						DataTable td = CernyForLifeTVDDAL.GetTVDPoradci(this.SourceDataStorage);
						foreach (DataRow row in td.Rows)
						{
							...
							if (row["Cislo_nadrizeneho"] == DBNull.Value) continue;
							...
							//decimal marze = CernyForLifeTVDDAL.GetTVDPoradcaMarze( this.SourceDataStorage, accountTVDId );
							decimal marze = 0m;
							...
							try
							{
								...
								CernyForLifeTVDDAL.UpdateTVDPoradciStav(this.SourceDataStorage, accountTVDId, 2);
