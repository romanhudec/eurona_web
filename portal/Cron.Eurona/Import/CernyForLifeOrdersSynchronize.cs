...
namespace Cron.Eurona.Import
{
		...
		/// </summary>
		public class CernyForLifeOrdersSynchronize: Synchronize
		{
				private int instanceId = 0;
				public CernyForLifeOrdersSynchronize( int instanceId, MSSQLStorage srcSqlStorage, MSSQLStorage dstSqlStorage )
				...
				public override void Synchronize()
				{
						...
						using ( SqlConnection connection = this.DestinationDataStorage.Connect() )
						{
								try
								{
										...
										try
										{
												...
												#region TVD->CERNYFORLIFE
												DataTable td = CernyForLifeTVDDAL.GetTVDFinalOrders( this.SourceDataStorage, this.InstanceId );
                        ...
                        SendEmail( errorMessage, sbMessage.ToString() );
												...
												#region TVD->CERNYFORLIFE ->MENA, sdeleni_pro_poradce_html
												td = CernyForLifeTVDDAL.GetTVDFakturyOrders( this.SourceDataStorage, this.InstanceId );
												foreach ( DataRow row in td.Rows )
												{
														int orderId = Convert.ToInt32( row["orderId"] );
														string kodMeny = GetString( row["Kod_meny"] );
