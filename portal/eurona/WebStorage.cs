...
namespace Eurona
{
	...
	public class WebStorage<T> : CMS.WebStorage<T> where T : class, new()
	{
		public override CMS.IStorage<T> Access()
		{
            ...
            if (typeof(T) == typeof(Eurona.Common.DAL.Entities.ZavozoveMisto)) access = new Eurona.Common.DAL.MSSQL.ZavozoveMistoStorage(instanceId, account, connectionString) as IStorage<T>;
