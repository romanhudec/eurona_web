<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="RequestEmailChangeFromAdminControl.ascx.cs" Inherits="Eurona.User.RequestEmailChangeFromAdminControl" %>
<script type="text/javascript">
    ...
    function onBtnVerifyClick() {
        ...
        var email = document.getElementById('<%=txtEmail.ClientID%>').value;
        $.ajax({
            url: "<%=Page.ResolveUrl("~/user/emailVerifycationService.ashx")%>?method=sendEmail2ChangeEmailFromAdmin",
            data: email + "|" + "<%=Page.Request["id"]%>",
            dataType: "json",
            type: "POST",
            contentType: "application/json; charset=utf-8",
            dataFilter: function (data) { return data; },
    ...
    function onClose() {
        location.href = "<%=Page.ResolveUrl("~/user/operator/")%>";
    }
</script>
