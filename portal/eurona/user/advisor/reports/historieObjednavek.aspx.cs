    ...
    public partial class HistorieObjednavek : ReportPage {
        ...
        private void GridViewDataBind(bool bind) {
            ...
            using (SqlConnection connection = tvdStorage.Connect()) {
                ...
                sql = @"SELECT f.id_prepoctu, cislo_objednavky = f.cislo_objednavky_eurosap, f.datum_vystaveni_objednavky, f.celkem_k_uhrade, celkem_bez_dph = f.zaklad_zs, f.dph_zs,
									...
									Stav_objednavky = ISNULL(ofr.StavK2,1),
									Stav_objednavky_nazev = 
										CASE ISNULL(ofr.StavK2,1) 
										WHEN 0 THEN 'Storno'
										WHEN 1 THEN 'Zpracováva se'
										WHEN 2 THEN 'Zpracováva se' 
										WHEN 4 THEN 'Vyřízena'
										ELSE ''
										END,
								...
								WHERE 
									...
									f.potvrzeno=1
									GROUP BY f.id_prepoctu, f.cislo_objednavky_eurosap, f.datum_vystaveni_objednavky, f.celkem_k_uhrade, f.zaklad_zs, f.dph_zs, ofr.StavK2,
