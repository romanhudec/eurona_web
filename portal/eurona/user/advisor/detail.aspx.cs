...
namespace Eurona.User.Advisor
{
		public partial class Detail: WebPage
		{
				protected void Page_Load( object sender, EventArgs e )
				{
						...
						{
								Person person = Storage<Person>.ReadFirst( new Person.ReadByAccountId { AccountId = Convert.ToInt32( Request["id"] ) } );
								if ( person != null )
								{
										this.personControl.Visible = true;
										this.personControl.PersonId = person.Id;
										this.hlBack.NavigateUrl = this.personControl.ReturnUrl;
										...
										this.personControl.Settings.HomeAddressSettings.Visibility.District = false;
										this.personControl.Settings.HomeAddressSettings.Visibility.Region = false;
								...
								Organization organization = Storage<Organization>.ReadFirst( new Organization.ReadByAccountId { AccountId = Convert.ToInt32( Request["id"] ) } );
