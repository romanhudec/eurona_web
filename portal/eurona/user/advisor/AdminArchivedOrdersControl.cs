...
namespace Eurona.User.Advisor
{
	public class AdminArchivedOrdersControl : CmsControl
	{
		...
		private RadGrid CreateGridView()
		{
            ...
            /*
			GridButtonColumn btnPay = new GridButtonColumn();
			btnPay.HeaderStyle.Width = new Unit(16, UnitType.Pixel);
			btnPay.ImageUrl = Page.ResolveUrl("~/images/e-pay.png");
			btnPay.Text = "Elektronická platba";
			btnPay.ButtonType = GridButtonColumnType.ImageButton;
			btnPay.CommandName = PAY_COMMAND;
			grid.MasterTableView.Columns.Add(btnPay);
            */
		...
		void OnRowDataBound(object sender, GridItemEventArgs e)
		{
            ...
            /*
			if (order.PaydDate.HasValue)
			{
				btnPay.Enabled = false;
				btnPay.ImageUrl = Page.ResolveUrl("~/images/e-pay_D.png");
			}
            */
		}
