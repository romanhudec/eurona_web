...
{
	public partial class Login : CmsControl
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			loginInfo.Visible = logged;
			if (logged)
			{
				info.Text = Security.Account.Login;
				info.NavigateUrl = Page.ResolveUrl("~/user/default.aspx?ReturnUrl=~/default.aspx");

				if (!Security.Account.IsInRole(Eurona.DAL.Entities.Role.ADMINISTRATOR))
				{
					//AccountCredit ac = Storage<AccountCredit>.ReadFirst( new AccountCredit.ReadByAccountId{ AccountId = Security.Account.Id});
					//creditInfo.Text = string.Format( Resources.Strings.LoginControl_YourCurrentCreditIs, ac == null ? 0 : (int)ac.Credit);
				}
			}
			...
			this.doRegister.NavigateUrl = alias;
		}
