...
namespace Eurona.Admin.Reports {
    public partial class SingleUserCookieReports2Page : WebPage {
      private int obdobi = 0;
        ...
        private void GridViewDataBind(bool bind) {
            ...
            using (SqlConnection connection = tvdStorage.Connect()) {
                ...
                string sql = @"
                    WITH Report (Obdobi, CookieAccountId, Pocet)
                    AS
                    (
	                    SELECT Obdobi=DATEPART(YEAR, sucla.Timestamp)*100+DATEPART(MONTH, sucla.Timestamp), sucla.CookieAccountId, Pocet=COUNT(sucla.RegistrationTimestamp)
	                    FROM tSingleUserCookieLinkActivity sucla
	                    WHERE sucla.RegistrationTimestamp IS NOT NULL
